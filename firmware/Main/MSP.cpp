...
bool MSP::recv(uint16_t *messageID, void *payload, uint16_t maxSize,
               uint16_t *recvSize) {
  uint32_t t0 = millis();
  uint8_t tmp_buf[2];
...
bool MSP::waitFor(uint16_t messageID, void *payload, uint16_t maxSize,
                  uint16_t *recvSize) {
  uint16_t recvMessageID;
  uint16_t recvSizeValue;
  uint32_t t0 = millis();
  while (millis() - t0 < _timeout)
    if (recv(&recvMessageID, payload, maxSize,
             (recvSize ? recvSize : &recvSizeValue)) &&
...
// recvSize can be NULL
bool MSP::request(uint16_t messageID, void *payload, uint16_t maxSize,
                  uint16_t *recvSize) {
...
bool MSP::command(uint16_t messageID, void *payload, uint16_t size,
                  bool waitACK) {
