...
#define MSP_RC_TUNING                                                          \
  111 // rc rate, rc expo, rollpitch rate, yaw rate, dyn throttle PID
...
#define MSP_NAV_STATUS_ERROR_INVALID_JUMP                                      \
  6 // Invalid jump target detected, aborting
...
struct msp_rx_map_t {
  uint8_t
      rxmap[MSP_MAX_MAPPABLE_RX_INPUTS]; // [0]=roll channel, [1]=pitch channel,
                                         // [2]=yaw channel, [3]=throttle
                                         // channel, [3+n]=aux n channel, etc...
} __attribute__((packed));
...
// MSP_SET_WP command
// Special waypoints are 0 and 255. 0 is the RTH position, 255 is the POSHOLD
...
struct msp_set_wp_t {
  ...
  int16_t p1;     // speed (cm/s) when action is
              // MSP_NAV_STATUS_WAYPOINT_ACTION_WAYPOINT, or "land" (value 1)
              // when action is MSP_NAV_STATUS_WAYPOINT_ACTION_RTH
  int16_t p2;   // not used
  int16_t p3;   // not used
...
public:
  ...
  bool recv(uint16_t *messageID, void *payload, uint16_t maxSize,
            uint16_t *recvSize);

  bool waitFor(uint16_t messageID, void *payload, uint16_t maxSize,
               uint16_t *recvSize = NULL);

  bool request(uint16_t messageID, void *payload, uint16_t maxSize,
               uint16_t *recvSize = NULL);

  bool command(uint16_t messageID, void *payload, uint16_t size,
               bool waitACK = true);
